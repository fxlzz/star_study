...
**解释：**
1. React 会将此**函数**加入队列，以便在事件处理函数中运行后进行处理
2. 在下一次渲染期间，*React 会遍历队列*并给你更新之后的最终 state
...
+ 事件处理函数执行完成后，React 将触发*重新渲染*
+ 在重新渲染期间，React 将*处理队列*
+ *更新函数*会在渲染期间执行，因此 **更新函数必须是** [纯函数](https://react.docschina.org/learn/keeping-components-pure) 并且只 **返回** 结果
...
1、`number、string、boolean`这些基本数据类型都是 **不可变的（immutable）**，修改值，让其指向新开辟的空间，原来的空间回收，**注意：不是在原来的空间上直接修改的，而是重新开辟了一个新的空间**

2、引用数据类型，是**可变**的，在指向的是**引用地址**，引用地址指向的才是真正的数据
...
### 用途
_useState_ 是用来声明状态的， _useEffect_ 是用来处理 **_副作用_** 的
...
适用场景：
需要根据 DOM 元素的尺寸来调整布局（比如自动计算 Tooltip 的位置）。
用 `useEffect` 可能会导致 UI 先跳一下再回到正确位置。`useLayoutEffect` 可以保证用户看到的就是最终的结果。

不要滥用 `useLayoutEffect`，毕竟是同步执行，可能会影响性能

---

## react-router
### 如何使用？
2026 年，`react-router-dom` 已经迭代到了 `v7` 版本，不仅仅是一个“前端路由库”，而是一个可以支持全栈开发、SSR 的“框架级路由”
官网： [React Router Home \| React Router](https://reactrouter.com/home)

有三种配置路由的方式
+ 框架模式 SSR
+ 数据模式 `createBrowserRouter`
+ 声明式 `<BrowserRouter> + <Routes>`
...
function UserProfile() {
 const user = useLoaderData(); // 自动获取 loader 返回的数据 
...
+ `NavLink` 有 `active` 样式
+ `Link` 没有样式
+ `Navigate` 导航组件 `<Navigate to="/xx", replace state={{xxx}} />` 
+ `redirect` 重定向
+ `useNavigate` hook

获取路由参数 hook：
+ `useParams`：获取动态参数 `path="/detail/:id"`
+ `useLocation`: 获取 [`window.location`](JS.md#`window.location`) 对象，支持获取 state 参数（传递额外的参数）

获取 state 参数：
```jsx
<Navigate to={login_path} replace state={{ form: location.pathname }} />

const location = useLocation();
console.log(location.state.from);
```

自定义路由守卫: [路由守卫](../场景/路由守卫.md)

---
### Data Mode 核心概念
+ `loader`： 在组件渲染前调用，返回一个 `Promise`。React Router 会自动等待完成后再渲染组件
+ `action`： 专门处理表单（`<Form>`）的提交。当表单提交时，对应的 `action` 执行

#### 对标 hook
| **Hook**        | **用途**                                                    |
| --------------- | --------------------------------------------------------- |
| `useLoaderData` | 获取当前路由 `loader` 返回的数据。                                    |
| `useActionData` | 获取最近一次 `action` 执行后的返回值（常用于表单错误提示）。                       |
| `useNavigation` | 获取全局导航状态（加载中/提交中），用于展示 Loading 条或禁用按钮。                    |
| `useSubmit`     | 手动触发表单提交。                                                 |
| `useFetcher`    | **极其重要：** 在不进行路由跳转的情况下，获取数据或触发 `action`（用于侧边栏、模态框、点赞功能等）。 |

### 最佳实践
>  给我的感觉，异步状态管理，不如用 `react-query`

#### 处理表单
使用 `react-router` 提供的 `<Form>` 组件代替原生 `<form>`。它不需要编写 `onSubmit`，提交动作会自动触发路由的 `action`
...
#### 状态反馈
利用 `useNavigation` 判断当前是处于加载状态还是提交状态：
...
高阶组件就是*接收一个组件，返回一个新组件*。用于对组件的横向抽离，抽离公共代码
...
但其实，高阶组件的出现，是为了解决类组件横向抽离而出现的。现在，以函数组件为主的 React，其实用*自定义 hook* 就可以实现逻辑复用了~
...
### _forwardRef_
向下传递 ref 参数，如果不用 `React.forwarfRef(comp)` 包裹组件，那么组件只会接收一个参数 props
...
`React.forwardRef(组件名)` 可以让父组件操作子组件，`useImperativeHandle` 允许子组件向父组件暴露内容（一般遵循单向数据流）
...
注意：
+ 使用 `useRef` 一般用于组件内部的 React 内置组件（input、button ...）
+ 如果绑定在自定义组件上，需要给自定义组件加上 `React.forwardRef` 将 ref 传递

## Context
可以创建一个**上下文环境**，用于解决*组件之间*共享数据的问题

聚焦三个问题：
+ 怎么创建上下文—— `createContext`
+ 怎么在上下文中*存*数据 —— `SomeContext.Provider`
+ 怎么在上下文中*取*数据 —— `useContext`
...
返回值：
- `SomeContext.Provider` 可以在上下文中**存储**数据
- `SomeContext.Consumer` 可以在上下文中**读取**数据【现在更加推荐用 `useContext` 】
...
function App() {
  ...
  return (
    <CountContext.Provider value={{count, setCount}}> // 存储数据
      <Page />;
...
function Page() {
  return (
    <CountContext.Consumer> // 读取数据 -> 用回调获取
      {
...
用于类组件中，这是一个生命周期函数，可以用 PureComponent 代替（不过现版本，废弃了）
...
其实底层实现，就是用套了一层 PureComponent，我们知道，PureCompoent 是看 props 和 state 是否发生变化。
只不过，函数组件的 state 因为 hook 的原因，优化了。所以只需要看 props 是否发生改变，就能知道要不要渲染组件。
...
为什么要缓存呢？
因为，函数组件中，每次修改了 state 会导致重新渲染组件，此时，组件中的函数都会被重新创建，**可能导致依赖该函数的子组件也重新渲染**。所以这个地方可以进行优化。
...
使用链表这种结构，有一个最大的好处就是在进行整颗树的对比（diff）计算时，这个过程是可以被打断。
...
>  
>  diff 算法比较的是 `currentFiberNode` 和 `react元素(createElement)`

### 三个假设
如果两颗树完全对比，算法复杂度是 `O(n3)` ，React 通过三个假设将其优化到了 `O(n)`
- 只对比*同一层级*的节点，不跨层级对比
- （同一层级）如果*组件类型*变了（比如 `div` 变 `p`），直接销毁重建，不再向下对比
...
如果*没有 key* 的话 *（p 变成 h3）*，react 会遵循规则二的情况，会直接删除 p 和 h3 元素，重新创建

如果*有 key* 的话，那么此时 dom 元素是可以复用的，react 会交换顺序，不会重新创建 dom 元素
...
比较复用的过程如下：
- *key 不同*
	- 无需判断 type，结果直接*无法复用*（如果有兄弟节点还需要判断兄弟节点）
- *key 相同*
...
单节点Diff 流程：
1. *比较 key*  -> key 相同——没设置 key=null
2. 判断 type -> 从 li 变成了 p，*type 不相同*
3. 结论： *无法复用*
...
特别注意：
key不相同，只能代表当前这个 fiberNode 不能复用，还需要遍历后续的兄弟节点
