...
## 核心概念
 1. "Shebang" (#!)
  脚本的第一行通常是 #!/bin/bash 或 #!/bin/sh。
   * `#` 是注释。
   * `!`读作 "bang"。
   * 这一行告诉系统：“嘿，请用 `/bin/bash` 这个解释器来执行下面的代码。”
   * 如果没有这一行，系统可能不知道该用谁来跑（比如可能会错误地用 Python 去跑）

2. 严格的空格规则
+ 变量两边，不允许出现空格，shell 会解析错误
+ `NAME="Jack"` 正确的 --> `NAME = "Jack"` 错误的
+ `[ "$A" == "$B" ]` 正确的

3. 变量引用
+ `$NAME` 变量引用
+ `${NAME}` 拼接字符串 -> `NAME="Apple" echo ${NAME}Pie -> 输出: ApplePie`
...
获取命令结果：`$(xxx)`
`basename 命令从完整路径里取最后的名字，比如 /.../apps/alert -> alert`
`app_name=$(basename "$dir")`

## 判断
`for ..in ;. do ... done`
JS 是 { ... } 包裹代码块，Shell 是用 do 和 done 包裹.
